...
class JsonRpcService : public KeyedService, public mojom::JsonRpcService {
 public:
  ...
  // Called by KeyringService when the user refreshes portfolio page.
  // Subject to client side rate limiting based on
  // kBraveWalletLastDiscoveredAssetsAt pref value. Only runs eth_getLogs
  // against block range between kBraveWalletAssetsDiscoveredThroughBlock pref
  // and "latest".
  void DiscoverAssetsOnAllSupportedChainsOnRefresh(  // todo make internal with
                                                     // callback param
      const std::vector<std::string>& account_addresses);
