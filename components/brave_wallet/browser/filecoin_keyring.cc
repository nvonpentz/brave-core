...
std::string FilecoinKeyring::ImportFilecoinBLSAccount(
  ...
  if (!hd_key)
    return std::string();
  auto protocol = mojom::FilecoinAddressProtocol::BLS;
  std::string address = network + std::to_string(int(protocol)) +
...
std::string FilecoinKeyring::ImportFilecoinSECP256K1Account(
  ...
  auto protocol = mojom::FilecoinAddressProtocol::SECP256K1;
  std::string address = network + std::to_string(int(protocol)) +
