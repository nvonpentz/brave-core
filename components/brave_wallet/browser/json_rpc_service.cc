...
void JsonRpcService::DiscoverAssetsOnAllSupportedChainsOnRefresh(
  ...
  auto asset_discovered_through_blocks =
      prefs_->GetDict(kBraveWalletAssetsDiscoveredThroughBlock).Clone();
  for (std::string chain_id : GetAssetDiscoverySupportedChains()) {
    ...
    // using the discovered through block as the start block
    // of the eth_getLogs query.
    ...
    if (discovered_through_block) {
      from_block = *discovered_through_block;
    }

    JsonRpcService::DiscoverAssets(chain_id, mojom::CoinType::ETH,
                                   account_addresses, true, from_block,
                                   to_block, std::move(internal_callback));
  }
}
...
// Call observers
void JsonRpcService::CompleteDiscoverAssets(
    const std::string& chain_id,
...
void JsonRpcService::DiscoverAssets(
  ...
  auto internal_callback =
      base::BindOnce(&JsonRpcService::OnGetAllTokensDiscoverAssets,
                     weak_ptr_factory_.GetWeakPtr(), chain_id,
                     account_addresses, std::move(user_assets), update_prefs,
...
void JsonRpcService::OnGetTransferLogs(
  ...
  if (update_prefs) {
    DictionaryPrefUpdate update(prefs_,
                                kBraveWalletAssetsDiscoveredThroughBlock);
    auto* assets_discovered_through_pref = update.Get()->GetIfDict();
    DCHECK(assets_discovered_through_pref);  // todo should I use dcheck
    const std::string* current =
        assets_discovered_through_pref->FindString(chain_id);
    uint256_t current_int = 0;
    if (current) {
      if (!HexValueToUint256(*current, &current_int)) {
        // issue converting saved value to a block
        JsonRpcService::CompleteDiscoverAssets(
            chain_id, std::vector<mojom::BlockchainTokenPtr>(),
            mojom::ProviderError::kInternalError,
            l10n_util::GetStringUTF8(IDS_WALLET_INTERNAL_ERROR),
    ...
    if (!current || current_int < largest_block) {
      assets_discovered_through_pref->Set(chain_id,
                                          Uint256ValueToHex(largest_block));
    }
  }
  JsonRpcService::CompleteDiscoverAssets(chain_id, std::move(discovered_assets),
                                         mojom::ProviderError::kSuccess, "",
                                         std::move(callback));
}
