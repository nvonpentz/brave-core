...
class AssetDiscoveryManagerUnitTest : public testing::Test {
 ...
 protected:
  ...
  void TestDiscoverAssets(
    ...
    asset_discovery_manager_->DiscoverAssets(chain_id, mojom::CoinType::ETH,
  ...
  void TestDiscoverAssetsOnAllSupportedChainsOnRefresh(
    ...
    asset_discovery_manager_->DiscoverAssetsOnAllSupportedChainsOnRefresh(
        account_addresses);
    base::RunLoop().RunUntilIdle();
  ...
  std::unique_ptr<TestBraveWalletServiceObserver> wallet_service_observer_;
...
TEST_F(AssetDiscoveryManagerUnitTest,
       DiscoverAssetsOnAllSupportedChainsOnRefresh) {
  // Send valid requests that yield no results and verify
  // 1. OnDiscoveredAssetsCompleted is called exactly once for each supported
  // chain
